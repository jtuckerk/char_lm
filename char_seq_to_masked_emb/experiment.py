...
def get_attr_list(inst, attr_list):
  for c in attr_list:
    inst = getattr(inst, c)
  return inst
...
def xentropy_loss_fn(output, labels):
  # TODO - slice the output earlier to prevent uncessacry work in the last few bert ops
...
def RunOne(h, model, data, mt, dev='cuda'):
  ...
  for m in h['freeze_modules']:
    if 'bert.' in m:
      ...
      module = get_attr_list(model, m.split("."))
  ...
  for name, param in model.named_parameters():
    if 'bert' not in name or param.requires_grad:
